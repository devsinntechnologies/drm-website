...
function PostCard({
  ...
  const accentVar = idx % 2 === 0 ? "var(--primary)" : "var(--secondary)";
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="block h-full">
      <motion.div
        ...
        className="glass-card rounded-[40px] border-gradient relative overflow-hidden group cursor-pointer flex flex-col shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:shadow-[0_30px_60px_rgba(var(--primary-rgb),0.2)] transition-all duration-500 bg-surface/40 backdrop-blur-2xl h-full"
        style={{ perspective: "800px" }}
      >
        <div
          className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
          style={{
            background: `linear-gradient(135deg, color-mix(in srgb, ${accentVar} 5%, transparent), transparent)`,
          }}
        />
        ...
        <div className="relative overflow-hidden aspect-[4/3] m-2 rounded-[32px] border border-surface-border/20 z-10">
          ...
          <div className="absolute top-4 left-4 flex items-center gap-2">
            <span
              className="tag-pill backdrop-blur-md"
              style={{
        ...
        <div className="p-4 flex-1 flex flex-col relative z-10">
          <span
            className="text-xs mb-3 font-black tracking-widest uppercase"

...
const fallbackPost = {
  ...
  content: `
    ...
    <p>The days of bulky, on-premise servers are over. Cloud-native systems built with <span class="text-black">Digi</span><span class="text-primary">Nizam</span> guarantee that your data is accessible anywhere, anytime, ensuring critical decision-making on the go.</p>
    ...
    <main className="min-h-screen bg-surface text-foreground py-4 md:py-6 relative overflow-hidden">
      ...
      <article className="max-w-4xl mx-auto px-4 lg:px-8 relative z-10">
        <div className="mb-4 md:mb-4">
          <Link href="/blog" className="inline-flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-wider hover:text-[var(--primary-hover)] transition-colors mb-4">
          ...
          <div className="flex items-center justify-between py-3 border-y border-surface-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[9px] uppercase font-bold text-muted tracking-wider"><span className="text-black font-black">Digi</span><span className="text-primary font-black">Nizam</span> Platform</div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-[11px] font-bold text-foreground">Published</div>
              <div className="text-[10px] uppercase font-bold text-muted">{post.date}</div>
            </div>
          </div>
        </div>
        ...
        <div className="blog-content max-w-3xl mx-auto">
          <style dangerouslySetInnerHTML={{
            ...
            .blog-content blockquote { border-left: 3px solid var(--primary); background: rgba(0, 85, 255, 0.05); padding: 1rem 1.25rem; border-top-right-radius: 0.75rem; border-bottom-right-radius: 0.75rem; color: var(--foreground); font-weight: 700; font-style: normal; margin: 1.5rem 0; font-size: 1rem; }
          `}} />
          <div dangerouslySetInnerHTML={{ __html: post.content }} />
        </div>

        {/* Footer actions */}
        <div className="mt-4 pt-6 border-t border-surface-border flex items-center justify-between">
          <div className="flex gap-3 items-center">
            <span className="text-muted font-bold text-xs">Share this insight:</span>
            {['Twitter', 'LinkedIn', 'Facebook'].map(net => (
              <button key={net} className="px-3 py-1.5 rounded-xl border border-surface-border bg-surface text-xs font-bold text-foreground hover:text-primary hover:border-primary transition-all shadow-sm">
                {net}
              </button>
            ))}
          </div>
